...
def parse_args(args):
    ...
    # second layer estimator mode: default, tune or resample
    sub_holdout_estimator_mode = p_holdout.add_subparsers(required=True, title="Estimator Mode", description="Valid subcommands")
    p_holdout_default = sub_holdout_estimator_mode.add_parser("default", parents=[make_base_parser(), make_extra_resample_parser(), make_holdout_parser()])
    p_holdout_tune = sub_holdout_estimator_mode.add_parser("tune", parents=[make_base_parser(), make_extra_resample_parser(), make_holdout_parser(), make_tune_parser()])
    ...
    p_cv_default = sub_cv_estimator_mode.add_parser("default", parents=[make_base_parser(), make_extra_resample_parser(), make_cv_parser()])
    p_cv_tune = sub_cv_estimator_mode.add_parser("tune", parents=[make_base_parser(), make_extra_resample_parser(), make_cv_parser(), make_tune_parser()])
