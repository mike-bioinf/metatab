...
def get_repetition_fold(iteration: int, pars: dict, logger: logging.Logger) -> tuple: 
    '''
    Utility to get the repetion and fold info based on the current iteration and splitting details (contained in pars).
    ...
    if pars["splitting_mode"] == "cv":
        ...
        logger.debug(f'Running on fold number {fold} of repetion number {repetition}:')
