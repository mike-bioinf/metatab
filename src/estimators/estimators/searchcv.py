...
from estimators.estimators.utils import fit_with_early_stop_on_validation_set
...
class SearchCV:
    ...
    @staticmethod
    def _get_add_string_to_params_func(clf_or_pipe: Classifier | Pipeline) -> Callable:
        '''
        Set as attribute the function that add the classifier name to the parameters.
        This function returns the dict of params if clf_or_pipe is not a Pipeline object.
        '''
        if isinstance(clf_or_pipe, Pipeline):
            name_classifier = clf_or_pipe.steps[-1][0]
            return lambda params: {f"{name_classifier}__{k}":v for k,v in params.items()}
        else:
