...
class DataLoader():
    ...
    The class infact implements {X/y}{ /_train/_test) attributes:
    ...
    def load_data(self, mode: Literal["df", "xy", "sets"], **load_params) -> None:
        '''
        Allow to call a specific 'load' method using the mode attribute.
        One must pass all the specific load method parameters.
        '''
        if mode == "df":
            self.load_data_df_mode(**load_params)
        elif mode == "xy":
            self.load_data_xy_mode(**load_params)
        elif mode == "sets":
            self.load_data_sets_mode(**load_params)
        else:
            raise NotImplementedError(f"mode must be one of 'df', 'xy' and 'sets'. '{mode}' is not supported.")


    
    def load_data_df_mode(
        self,
        path: str | Path, 
        target_feature: str, 
        ...
        **kwargs
        ...
        Parameters:
            ...
            load_as (Literal["generic", "train", "test"]): Specify where to store the data in terms of attributes.
        ...
        **kwargs
        ...
        for set_name in sets_names:
            ...
            if not set_path.exists():
                if (isinstance(save_missing, bool) and save_missing) or (isinstance(save_missing, list) and set_path in save_missing):
                    continue
                else:
    ...
    def _set_xy_attributes(self, x_value: Any, y_value: Any, load_type: Literal["generic", "train", "test"]) -> None:
