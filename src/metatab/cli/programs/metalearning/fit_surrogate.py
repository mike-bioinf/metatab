...
def parse_args(args):
    ...
    p.add_argument("-e", "--estimator", required=True, 
                   choices=["random_forest", "extra_trees", "xgb", "es_xgb", "catboost", "es_catboost", "lgbm", "es_lgbm", "tabpfn", "realmlp", "tabm"],
...
def log_program_setting(logger: Logger, pars: dict):
    logger.debug(
        (
            f"\nLaunching the surrogate fit program on folder '{pars["meta_folder"].name}'"
